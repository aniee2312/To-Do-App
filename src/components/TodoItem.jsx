...
<div className="flex justify-between item-center gap-2">
        <label className={`hover:bg-slate-100 flex-1 p-2
        rounded-md cursor-pointer select-none ${isComplete?"line-through":""}`} 
        onClick={()=>toggleTask(id)}>{text}</label>
        <div>
        <div className="size-[26px]p-1 hover:bg-red-50 rounded-md"onClick={()=>deleteTodo(id)}>            
        <svg className="hover:fill-red-700" xmlns="http://www.w3.org/2000/svg" 
